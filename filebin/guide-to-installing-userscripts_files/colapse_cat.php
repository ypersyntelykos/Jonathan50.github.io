var var_cookieid = "mymagicalmodforums$";
var var_cookie_domain = "";
var var_cookie_path = "/forums/mymagicalmodforums/";
...
function togglecategory(fid)
{
	...
	if (my_getbyid('box_'+fid).style.display == "")
	{
		...
		my_getbyid('img_'+fid).src = my_getbyid('img_'+fid).src.replace('exp_up','exp_down');
	}
	else
	{
		my_getbyid('box_'+fid).style.display = "";
		my_getbyid('img_'+fid).src = my_getbyid('img_'+fid).src.replace('exp_down','exp_up');
	}
